...
export async function GET(request: Request) {
    try {
        ...
        const { data: assets } = await supabase
            .from('assets')
            .select('current_value, value_sgd, category:asset_categories(type)');
        ...
        const currentNw = (assets || []).length > 0 ? totalAssets - totalLiabilities : null;
        ...
        const lastSnapshotNw = history?.[0]?.net_worth ?? null;
        const lastSnapshotDate = history?.[0]?.snapshot_date ?? null;
        const nwChange =
            currentNw != null && lastSnapshotNw != null ? currentNw - Number(lastSnapshotNw) : null;
        ...
        const dataLines = [
            ...
            currentNw != null
                ? `Current net worth: ${currentNw.toFixed(0)}; change vs last snapshot${lastSnapshotDate ? ` (${lastSnapshotDate})` : ''}: ${nwChange != null ? `${nwChange >= 0 ? '+' : ''}${nwChange.toFixed(0)}` : 'n/a'}.`
